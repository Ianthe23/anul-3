...
// Structura pentru nodul din arborele binar de cautare
struct SymbolNode {
    string symbol;
    int index;
    SymbolNode* left;
    ...
    SymbolNode(const string& sym, int idx) : symbol(sym), index(idx), left(nullptr), right(nullptr) {}
};

// Clasa pentru tabela de simboluri (arbore binar de cautare)
class SymbolTable {
private:
    SymbolNode* root;
    int nextIndex;
    
    // Functie auxiliara pentru inserare in arbore
    ...
    // Functie auxiliara pentru cautare in arbore
    ...
    // Functie auxiliara pentru parcurgere inordine a arborelui
...
public:
    SymbolTable() : root(nullptr), nextIndex(0) {}
...
// Structura pentru FIP (Forma Interna a Programului)
struct FIPEntry {
    string tokenType;
    int code;
