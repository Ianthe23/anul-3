...
// Dimensiuni citite din fișier
int N = 0, M = 0;   // matrice F și V: N x M
int KH = 0, KW = 0; // nucleu C: KH x KW (n x m)
int P = 1;          // număr threaduri (din fișier; se poate suprascrie din CLI)

// Matrici alocate dinamic (contiguous)
int* F = nullptr; // N*M
int* V = nullptr; // N*M
int* C = nullptr; // KH*KW

inline int idxF(int i, int j) { return i * M + j; }
inline int idxC(int a, int b) { return a * KW + b; }
...
// Citește datele din 'date.txt' cu format:
// N M
// KH KW
// P
...
inline int convolve_cell(int i, int j) {
    ...
    for (int a = 0; a < KH; ++a) {
        ...
        for (int b = 0; b < KW; ++b) {
            ...
            sum += static_cast<long long>(F[idxF(ii, jj)]) * C[idxC(a, b)];
        }
    }
